...
var text_queue = []
var limite = false
onready var anima_porta = get_parent().get_parent().get_node("anima_porta")
func cadeado_chave():
	if senha == null:
	...
	if senha == senha_coparacao:
		...
		yield(anima_porta,"animation_finished")
...
func _on_chave_de_entrada_body_entered(body):
...
func _on_chave_de_entrada_body_exited(body):
	hide()
