...
func _ready():
	_Text()
	queue_text("Pressed 'W' andar para Cima")
	queue_text("Pressed 'S' andar para Baixo")
	queue_text("Pressed 'A' andar para Esquerda")
	queue_text("Pressed 'D' andar para Direita")
	queue_text("Pressed 'Space' para Interagir")
func _process(delta):
	match corrent_state:
		State.READY:
			if !text_queue.empty():
